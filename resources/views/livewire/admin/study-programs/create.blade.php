<div>
    ...
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-6">
        <div>
            <h1 class="text-2xl font-bold text-gray-900">Tambah Program Studi</h1>
            <p class="text-gray-600 mt-1">Buat program studi baru</p>
        </div>
                ...
                <x-mary-card title="Informasi Dasar" subtitle="Data utama program studi">
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                        ...
                        <div class="md:col-span-1">
                            <x-mary-input 
                                wire:model.blur="kode"
                                label="Kode Program *"
                                placeholder="Contoh: TKJ, RPL, MM"
                                hint="Kode akan otomatis ter-generate saat nama diisi. Kode unik untuk program studi (huruf kapital dan angka)"
                                maxlength="10" />
                            <div class="mt-2 flex items-center gap-2">
                                <x-mary-button 
                                    label="Generate Ulang" 
                                    icon="o-sparkles" 
                                    wire:click="generateKode"
                    ...
                    <x-mary-input 
                        ...
                        hint="Nama lengkap program studi"
                        maxlength="255" />
                    ...
                    <x-mary-textarea 
                        ...
                        hint="Penjelasan singkat tentang program studi (opsional)"
                        rows="4"
                        maxlength="5000" />

                    {{-- Ketua Program --}}
                    <x-mary-input 
                        wire:model.blur="ketua_program"
                        label="Ketua Program"
                        placeholder="Nama ketua program studi"
                        hint="Nama ketua program studi (opsional)"
                        maxlength="255" />
                </x-mary-card>
                    ...
                    <div class="space-y-3">
                        @foreach($kompetensi as $index => $item)
                            <div class="flex gap-2 items-start">
                                <div class="flex-1">
                                    <x-mary-input 
                                        wire:model.blur="kompetensi.{{ $index }}"
                                ...
                                <div class="flex gap-1 mt-2">
                                    ...
                                    @if(count($kompetensi) > 1)
                                        <x-mary-button 
                                            icon="o-trash" 
                                            wire:click="removeKompetensi({{ $index }})"
                    ...
                    <div class="space-y-3">
                        @foreach($prospek_karir as $index => $item)
                            <div class="flex gap-2 items-start">
                                <div class="flex-1">
                                    <x-mary-input 
                                        wire:model.blur="prospek_karir.{{ $index }}"
                                ...
                                <div class="flex gap-1 mt-2">
                                    @if($index === count($prospek_karir) - 1)
                                        <x-mary-button 
                                            icon="o-plus" 
                                            wire:click="addProspekKarir"
                                            class="btn-ghost btn-sm"
                                            type="button"
                                            tooltip="Tambah prospek karir" />
                                    @endif
            ...
            <div class="space-y-6">
                ...
                <x-mary-card title="Warna Tema" subtitle="Pilih warna representatif">
                    <div class="space-y-4">
                        ...
                        <div class="flex items-center gap-3">
                            <div class="w-12 h-12 rounded-lg border border-gray-200" style="background-color: {{ $warna }}"></div>
                            <div>
                        ...
                        <div class="grid grid-cols-6 gap-2">
                            @php
                                $presetColors = ['#3b82f6', '#ef4444', '#10b981', '#f59e0b', '#8b5cf6', '#06b6d4', '#84cc16', '#f97316', '#ec4899', '#6b7280'];
                            @endphp
                            @foreach($presetColors as $color)
                                <button 
                                    type="button"
                                    wire:click="$set('warna', '{{ $color }}')"
                ...
                <x-mary-card title="Status" subtitle="Pengaturan status program">
                    <x-mary-toggle 
                        ...
                        label="Program Aktif"
                        hint="Program studi dapat dilihat di website"
